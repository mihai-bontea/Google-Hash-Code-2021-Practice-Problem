...
public:
	...
	void add_pizza(list<Pizza>::iterator pizza_it)
	{
		...
		score = (double)nr_ingredients;
	}

	double replace_profit(const Pizza &pizza, int replace_index)
	{
		...
		for (int ingr_index = 0; ingr_index < total_unique_ingr; ++ingr_index)
		{
			...
			ingr_count += pizza.ingr_is_present[ingr_index];
		...
		new_score = (double)new_nr_ingredients;
		return new_score - score;
	}

	bool swap_if_profit(Delivery &other)
	{
		for (int i = 0; i < nr_pizzas; ++i)
			for (int j = i; j < other.nr_pizzas; ++j)
			{
				double profit1 = replace_profit(other.pizzas[j], i);
				double profit2 = other.replace_profit(pizzas[i], j);

				if (profit1 + profit2 > 0)
				{
					Pizza aux = pizzas[i];
					replace_index(other.pizzas[j], i);
					other.replace_index(aux, j);
					return true;
				}
			}
		return false;
	...
	void replace_index(Pizza pizza, int replace_index)
	{
		pizzas[replace_index] = pizza;
